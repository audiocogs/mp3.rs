...
fn decode_bit_allocations(bit_reader: &mut bitreader::BitReader, num_subbands: u32, num_channels: u32) -> Box<[[u32; 32]; 2]> {
...
fn decode_scale_factors(bit_reader: &mut bitreader::BitReader, num_subbands: u32, num_channels: u32, allocations: &Box<[[u32; 32]; 2]>) -> Box<[[u32; 32]; 2]> {
...
fn decode_samples(bit_reader: &mut bitreader::BitReader, num_subbands: u32, num_channels: u32, allocations: &Box<[[u32; 32]; 2]>, scale_factors: &Box<[[u32; 32]; 2]>) -> Box<[[[f64; 32]; 12]; 2]> {
    ...
    for subband in 0..num_subbands {
      for channel in 0..num_channels {
        ...
        samples[channel][sample][subband] = if nb > 0 {
          calculate_sample(bit_reader, nb as u32) * SCALE_FACTORS_TABLE[scale_factors[channel][subband] as u32]
        } else {
          0.0
        };
      }
...
fn calculate_sample(bit_reader: &mut bitreader::BitReader, nb: u32) -> f64 {
